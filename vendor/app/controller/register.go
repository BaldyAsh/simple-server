...
func RegisterPOST(w http.ResponseWriter, r *http.Request) {
	...
	password, errp := hash.HashString(r.FormValue("password"))
	...
	// Get database result
	_, err := dbmodel.UserByEmail(email)

	if err == dbmodel.ErrNoResult { // If success (no user exists with that email)
		ex := dbmodel.UserCreate(firstName, lastName, email, password)

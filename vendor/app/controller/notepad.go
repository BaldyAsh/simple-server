...
func NotepadReadGET(w http.ResponseWriter, r *http.Request) {
	...
	notes, err := dbmodel.NotesByUserID(userID)
	if err != nil {
		log.Println(err)
		notes = []dbmodel.Note{}
	}
...
func NotepadCreateGET(w http.ResponseWriter, r *http.Request) {
	...
	v.Vars["token"] = csrf.Token(w, r, sess)
	v.Render(w)
}
...
func NotepadCreatePOST(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := dbmodel.NoteCreate(content, userID)
...
func NotepadUpdateGET(w http.ResponseWriter, r *http.Request) {
	...
	params = context.Get(r, "params").(router.Params)
	noteID := params.ByName("id")

	userID := fmt.Sprintf("%s", sess.Values["id"])

	// Get the note
	note, err := dbmodel.NoteByID(userID, noteID)
...
func NotepadUpdatePOST(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := dbmodel.NoteUpdate(content, userID, noteID)
	// Will only error if there is a problem with the query
	if err != nil {
		log.Println(err)
		sess.AddFlash(view.Flash{"An error occurred on the server. Please try again later.", view.FlashError})
...
func NotepadDeleteGET(w http.ResponseWriter, r *http.Request) {
	...
	// Get database result
	err := dbmodel.NoteDelete(userID, noteID)
	// Will only error if there is a problem with the query
	if err != nil {
		log.Println(err)
		sess.AddFlash(view.Flash{"An error occurred on the server. Please try again later.", view.FlashError})
